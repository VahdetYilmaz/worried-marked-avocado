...
exports.run = async (bot, message, args) => {
  if (!args[0]) return message.channel.send(`:x: Aç yada kapat yazmalısınm! Örnek: \`${pref}sa-as aç\``)
  ...
  if (args[0] == 'aç') {
    db.set(`saas_${message.guild.id}`, 'acik').then(i => {
     return message.channel.sendEmbed(new Discord.RichEmbed().setDescription(`<a:onaylandI:586709716101496862> Artık biri sa diyince otomatik olarak Aleyküm Selam diyecek.`).setColor("RANDOM"));
    })
  }
  if (args[0] == 'kapat') {
