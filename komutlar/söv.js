exports.run = async (client, msg, args) => {
  let kufur=[
    ...
    "**Ananın amına trafo sokar ülkenin elektrik ihtiyacını karsılatırım**",
  ]
 ...
 description: (`:no_entry_sign: Hoop! Yapımcıma Sövemezsin!!!`)
   ...
   if(member.id === "756966543275065475")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Bana Sövemezsin!!!`)
}}) 
  if(member.id === "729074912274153533")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop Yapımcıma Sövemezsin!!!`)
}}) 
...
exports.help = {
  name: 'söv',
  description: 'Etiketlediğiniz Kişiye Söver!',
  usage: 'söv'
 }