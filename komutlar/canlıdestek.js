...
const emoji7 = message.client.emojis.get('📞');
      ...
      let destekKanal = "717386481101963426";
      const embed = new Discord.RichEmbed()
        .addField('Uyarı', `<a:ZilGif:714416721569513503> Canlı Destek Çağrısı`)
    ...
    collector.on('end', (collected, reason) => {
      ...
      if (reason === 'success') {
        ...
        chan.send('Çağrınız bir destek yetkili tarafından alındı! En Yakın Zamanda Size Yardımcı Olacagız.')
        chan.send('<a:loa:711681915672854610>')
        ...
        client.on('message', message => {
          function contact() {
            ...
            if (message.content.startsWith('kapat')) {
              message.channel.send('Çağrı Kapatıldı.<a:ElSallamaGif:714413917627219988>')
              if (message.channel.id === chan.id) client.channels.get(destekKanal).send('Çağrı karşı taraftan kapatıldı.<a:ElSallamaGif:714413917627219988>')
              if (message.channel.id === destekKanal) chan.send('Çağrı karşı taraftan kapatıldı.<a:ElSallamaGif:714413917627219988>')
